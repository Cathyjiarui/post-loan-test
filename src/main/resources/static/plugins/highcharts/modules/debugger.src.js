    ...
    _registerModule(_modules, 'error-messages.js', [_modules['parts/Globals.js']], function (H) {
        ...
        H.errorMessages = {
            "10": {
                ...
                "text": "<h1>Can't plot zero or subzero values on a logarithmic axis</h1><p>This error occurs in the following situations: </p><ul><li>If a zero or subzero data value is added to a logarithmic axis</li><li>If the minimum of a logarithimic axis is set to 0 or less</li><li>If the threshold is set to 0 or less</li></ul><p>Note: As of Highcharts 5.0.8 it's possible to bypass this error message by setting <highcharts>Axis.prototype.allowNegativeLog</highcharts> to true, and add custom conversion functions. <a href=\"https://jsfiddle.net/gh/get/library/pure/highcharts/highcharts/tree/samples/highcharts/yaxis/type-log-negative/\">View live demo</a>.</p>",
                "enduser": "<h1>Can't plot zero or subzero values on a logarithmic axis</h1><p>This error occurs in the following situations:</p><p>* If a zero or subzero data value is added to a logarithmic axis</p><p>* If the minimum of a logarithimic axis is set to 0 or less</p><p>* If the threshold is set to 0 or less</p><p>As of Highcharts 5.0.8 it's possible to bypass this error message by setting <highcharts>Axis.prototype.allowNegativeLog</highcharts> to <highcharts>true</highcharts> and add custom conversion functions. <a href=\"http://jsfiddle.net/gh/get/library/pure/highcharts/highcharts/tree/samples/highcharts/yaxis/type-log-negative/\">\">View Live Demo</a>.</p>"
            },
            "11": {
                "title": "Can't link axes of different type",
                "text": "<h1>Can't link axes of different type</h1><p>This error occurs when using the <highcharts>linkedTo</highcharts> option to link two axes of different types, for example a logarithmic axis to a linear axis. Highcharts can't link these because the calculation of ticks, extremes, padding etc. is different.</p>"
            },
            "12": {
                "title": "Highcharts expects point configuration to be numbers or arrays in turbo mode",
                "text": "<h1>Highcharts expects point configuration to be numbers or arrays in turbo mode</h1><p>This error occurs if the series.data option contains object configurations and the number of points exceeds the turboThreshold. It can be fixed by either setting <highcharts>turboThreshold</highcharts> to a higher value, or changing the point configurations to numbers or arrays.</p><p>See <a href=\"https://api.highcharts.com/highcharts#plotOptions.series.turboThreshold\">plotOptions.series.turboThreshold</a></p>"
            },
            "13": {
            ...
            "16": {
                ...
                "text": "<h1>Highcharts already defined in the page</h1><p>This error happens if the <highcharts>Highcharts</highcharts> namespace already exists when loading</p><p>Highcharts or Highstock.</p><p>This is caused by including Highcharts or Highstock more than once.</p><p>Keep in mind that the <highcharts>Highcharts.Chart</highcharts> constructor and all features of Highcharts are included in Highstock, so if using the <highcharts>Chart</highcharts> and <highcharts>StockChart</highcharts> constructors in combination, only the <highcharts>highstock.js</highcharts> file is required.</p>"
            },
            "17": {
                "title": "The requested series type does not exist",
                "text": "<h1>The requested series type does not exist</h1><p>This error happens when setting <highcharts>chart.type</highcharts> or <highcharts>series.type</highcharts> to a series type that isn't defined in Highcharts. A typical reason may be that the</p><p>module or extension where the series type is defined isn't included.</p><p>For example in order to create an <highcharts>arearange</highcharts> series, the <highcharts>highcharts-more.js</highcharts> file must be loaded.</p>"
            },
            "18": {
                "title": "The requested axis does not exist",
                "text": "<h1>The requested axis does not exist</h1><p>This error happens when setting a series' <highcharts>xAxis</highcharts> or <highcharts>yAxis</highcharts> property to point to an axis that does not exist.</p>"
            },
            "19": {
                "title": "Too many ticks",
                "text": "<h1>Too many ticks</h1><p>This error happens when applying too many ticks to an axis, specifically when adding more ticks than the axis pixel length.</p><p>With default value this won't happen, but there are edge cases, for example when setting axis categories and <highcharts>xAxis.labels.step</highcharts> in combination with a long data range, when the axis is instructed to create a great number of ticks.</p>"
            },
            "20": {
            ...
            "22": {
                ...
                "text": "<h1>Map does not support latitude/longitude</h1><p>The loaded map does not support latitude/longitude functionality. This is only supported with maps from the <a href=\"https://highcharts.highcharts.com/mapdata\">official Highmaps map collection</a> from version 1.1.0 onwards. If you are using a custom map, consider using the <a href=\"https://proj4js.org\">Proj4js</a> library to convert between projections.</p>"
            },
            "23": {
                "title": "Unsupported color format used for color interpolation",
                "text": "<h1>Unsupported color format used for color interpolation</h1><p>Highcharts supports three color formats primarily: hex (`#FFFFFF`), rgb (<highcharts>rgba(255,255,255)</highcharts>) and rgba (<highcharts>rgba(255,255,255,1)</highcharts>). If any other format, like 3-digit colors (`#FFF`), named colors (<highcharts>white</highcharts>) or gradient structures are used in for example a heatmap, Highcharts will fail to interpolate and will</p><p>instead use the end-color with no interpolation applied.</p><p>We've chosen to preserve this limitation in order to keep the weight of the implementation at a minimum.</p>"
            },
            "24": {
                "title": "Cannot run Point.update on a grouped point",
                "text": "<h1>Cannot run Point.update on a grouped point</h1><p>Running <highcharts>Point.update</highcharts> in Highstock when a point is grouped by data grouping is not supported.</p><p>This is not supported because when data grouping is enabled, there won't be any</p><p>references to the raw points, which is required by the <highcharts>Point.update</highcharts> function.</p>"
            },
            "25": {
            ...
            "26": {
                ...
                "text": "<h1>WebGL not supported, and no fallback module included</h1><p>This happens when the browser doesn't support WebGL,<b>and</b> the canvas fallback module (<highcharts>boost-canvas.js</highcharts>) hasn't been included OR if the fallback module was included<b>after</b> the boost module.</p><p>If a fallback is required, make sure to include <highcharts>boost-canvas.js</highcharts>, and that it's included before <highcharts>boost.js</highcharts>.</p><p>Please note that the fallback module is not intended as a fully-featured one. Rather, it's a minimal implementation of the WebGL counterpart.</p>"
            },
            "27": {
                "title": "This browser does not support SVG",
                "text": "<h1>This browser does not support SVG</h1><p>This happens in old IE when the <highcharts>oldie.js</highcharts> module isn't loaded.</p><p>If compatibility with IE versions 6, 7 and 8 is required, add the module after</p><p>loading <highcharts>highcharts.js</highcharts>. In a website context, it's a good idea to load it in a conditional comment to avoid traffic overhead and dead highcharts in modern browsers: </p><pre>&lt;!--[if lt IE 9]&gt;\n    &lt;script src='https://highcharts.highcharts.com/modules/oldie.js'&gt;&lt;/script&gt;\n&lt;![endif]--&gt;</pre>"
            },
            "28": {
            ...
            "31": {
                ...
                "text": "<h1>Non-unique point or node id</h1><p>This error occurs when using the same <highcharts>id</highcharts> for two or more points or nodes.</p>"
            },
            "meta": {
                "files": [
                    "errors/10/readme.md",
