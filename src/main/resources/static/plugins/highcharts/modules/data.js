...
*/
(function (m) {
    "object" === typeof module && module.exports ? (m["default"] = m, module.exports = m) : "function" === typeof define && define.amd ? define("highcharts/modules/data", ["highcharts"], function (x) {
        m(x);
        m.Highcharts = x;
        return m
    ...
    x(m, "modules/data.src.js", [m["parts/Globals.js"]], function (g) {
        var m = g.addEvent, n = g.Chart, p = g.win.document, x = g.objectEach, C = g.pick, D = g.isNumber, z = g.merge,
            F = g.splat, G = g.fireEvent, y, A = function (a, b, d) {
                this.init(a, b, d)
            };
        g.extend(A.prototype, {
            init: function (a, b, d) {
                ...
                var a = this.chartOptions, b = this.options, d = [], e = function (a) {
                    ...
                    }, f = a && a.chart && a.chart.type, c = [], k = [], q = 0,
                ...
                b.forEach(function (b) {
                    ...
                    for (h = 0; h < m; h++) d.hasReader(t[h]) || d.addColumnReader(void 0, t[h]);
                    k.push(d);
                    q++
                });
                ...
                function b(a, b, d, c) {
                    ...
                    function h() {
                        g > n || n > m ? (++n, w = "") : (!isNaN(parseFloat(w)) && isFinite(w) ? (w = parseFloat(w), e("number")) : isNaN(Date.parse(w)) ? e("string") : (w = w.replace(/\//g, "-"), e("date")), q.length < t + 1 && q.push([]), d || (q[t][b] = w), w = "", ++t, ++n)
                    }
                ...
                function d(a) {
                    ...
                    a.some(function (a, c) {
                        ...
                        for (var h = 0; h < a.length; h++) {
                            ...
                            else "undefined" !== typeof v[c] ? (k = k.trim(), isNaN(Date.parse(k)) ? !isNaN(k) && isFinite(k) || v[c]++ : v[c]++, k = "") : k += c;
                            "," === c && d++;
                            "." === c && b++
                        }
                    });
                ...
                function e(a, b) {
                    ...
                    for (; k < b; k++) if ("undefined" !== typeof a[k] && a[k] && a[k].length) for (d = a[k].trim().replace(/\//g, " ").replace(/\-/g, " ").replace(/\./g, " ").split(" "),
                                                                                                        e = ["", "", ""], l = 0; l < d.length; l++) l < e.length && (d[l] = parseInt(d[l], 10), d[l] && (g[l] = !g[l] || g[l] < d[l] ? d[l] : g[l], "undefined" !== typeof q[l] ? q[l] !== d[l] && (q[l] = !1) : q[l] = d[l], 31 < d[l] ? e[l] = 100 > d[l] ? "YY" : "YYYY" : 12 < d[l] && 31 >= d[l] ? (e[l] = "dd", h = !0) : e[l].length || (e[l] = "mm")));
                    if (h) {
                        for (l = 0; l < q.length; l++) !1 !== q[l] ? 12 < g[l] && "YY" !== e[l] && "YYYY" !== e[l] && (e[l] = "YY") : 12 < g[l] && "mm" === e[l] && (e[l] = "dd");
                ...
                var h = c.endRow || Number.MAX_VALUE,
                    g = "undefined" !== typeof c.startColumn && c.startColumn ? c.startColumn : 0,
                    m = c.endColumn || Number.MAX_VALUE, B, t = 0, u = [], v = {",": 0, ";": 0, "\t": 0};
                q = this.columns = [];
                ...
                if (k) {
                    ...
                    for (var n = 0, t = a; t <= h; t++) "#" === k[t][0] ? n++ : b(k[t], t - a - n);
                    c.columnTypes && 0 !== c.columnTypes.length || !u.length || !u[0].length || "date" !== u[0][1] || c.dateFormat || (c.dateFormat = e(q[0]));
                    this.dataFound()
                }
                return q
            }, parseTable: function () {
                var a = this.options, b = a.table, d = this.columns, e = a.startRow || 0,
                    f = a.endRow || Number.MAX_VALUE, c = a.startColumn || 0, k = a.endColumn || Number.MAX_VALUE;
                b && ("string" === typeof b && (b = p.getElementById(b)), [].forEach.call(b.getElementsByTagName("tr"), function (a,
                                                                                                                                  b) {
                    b >= e && b <= f && [].forEach.call(a.children, function (a, f) {
                ...
                function a(h) {
                    function m(q, m, t) {
                        ...
                        if (!q || 0 !== q.indexOf("http")) return q && e.error && e.error("Invalid URL"), !1;
                    ...
                    m(q.csvURL, function (a) {
                        d.update({data: {csv: a}})
                    }, "text") || m(q.rowsURL, function (a) {
                ...
                var b = this, d = this.chart, e = this.options, f = 0, c = e.enablePolling,
                    k = 1E3 * (e.dataRefreshRate || 2), q = z(e);
                ...
                function a(b) {
                    var f =
                        ["https://spreadsheets.google.com/feeds/cells", e, c, "public/values?alt\x3djson"].join("/");
                    g.ajax({
                        url: f, dataType: "json", success: function (c) {
                            b(c);
                            d.enablePolling && setTimeout(function () {
                                a(b)
                            }, 1E3 * (d.dataRefreshRate || 2))
                        }, error: function (a, b) {
                ...
                var b = this, d = this.options, e = d.googleSpreadsheetKey, f = this.chart,
                    c = d.googleSpreadsheetWorksheet || 1, k = d.startRow || 0, q = d.endRow || Number.MAX_VALUE,
                    h = d.startColumn || 0, m = d.endColumn || Number.MAX_VALUE, n = 1E3 * (d.dataRefreshRate || 2);
                4E3 > n &&
                (n = 4E3);
                e && (delete d.googleSpreadsheetKey, a(function (a) {
                    var d = [];
                    a = a.feed.entry;
                    ...
                    for (r = 0; r < e; r++) c = a[r], g = c.gs$cell.row - 1, n = c.gs$cell.col - 1, n >= h && n <= m && g >= k && g <= q && (p = c.gs$cell || c.content, c = null, p.numericValue ? c = 0 <= p.$t.indexOf("/") || 0 <= p.$t.indexOf("-") ? p.$t : 0 < p.$t.indexOf("%") ? 100 * parseFloat(p.numericValue) : parseFloat(p.numericValue) : p.$t && p.$t.length &&
                        (c = p.$t), d[n - h][g - k] = c);
                ...
                var d = this.rawColumns,
                    e = this.columns, f = a.length, c, k, g, h, m = this.firstRowAsNames,
                    n = -1 !== this.valueCount.xColumns.indexOf(b), p, t = [], u = this.chartOptions, v,
                    x = (this.options.columnTypes || [])[b],
                    u = n && (u && u.xAxis && "category" === F(u.xAxis)[0].type || "string" === x);
                for (d[b] || (d[b] = []); f--;) c = t[f] || a[f], g = this.trim(c), h = this.trim(c, !0), k = parseFloat(h), void 0 === d[b][f] && (d[b][f] = g), u || 0 === f && m ? a[f] = "" + g : +h === k ? (a[f] = k, 31536E6 < k && "float" !== x ? a.isDatetime = !0 : a.isNumeric = !0, void 0 !== a[f + 1] && (v = k > a[f + 1])) : (g && g.length && (p = this.parseDate(c)),
                ...
                n && a.mixed && (e[b] = d[b]);
                if (n && v && this.options.sort) for (b = 0; b < e.length; b++) e[b].reverse(), m && e[b].unshift(e[b].pop())
            }, dateFormats: {
                "YYYY/mm/dd": {
                ...
                for (a = 0; a < b.length; a += 1) for (c = b[a].getReferencedColumnIndexes(), d = 0; d < c.length; d += 1) e[c[d]] = !1;
                for (d = 0; d < e.length; d += 1) e[d] && f.push(d);
                return f
            }, complete: function () {
                var a = this.columns, b, d = this.options, e, f, c, k, g = [], h;
                if (d.complete || d.afterComplete) {
                    if (this.firstRowAsNames) for (c = 0; c < a.length; c++) a[c].name = a[c].shift();
                    e = [];
                    f = this.getFreeIndexes(a.length, this.valueCount.seriesBuilders);
                    for (c = 0; c < this.valueCount.seriesBuilders.length; c++) h =
                    ...
                    if ("category" === b) for (c = 0; c < g.length; c++) for (h = g[c], f = 0; f < h.readers.length; f++) "x" ===
        ...
        m(n, "init", function (a) {
            ...
            d && d.data && !b.hasDataDef && (b.hasDataDef = !0, b.data = new A(g.extend(d.data, {
                afterComplete: function (a) {
                    var c, f;
                    if (d.hasOwnProperty("series")) if ("object" === typeof d.series) for (c = Math.max(d.series.length, a && a.series ? a.series.length : 0); c--;) f = d.series[c] || {}, d.series[c] = z(f, a && a.series ? a.series[c] : {}); else delete d.series;
                    d = z(a,
                        d);
                    b.init(d, e)
                }
        ...
        y.prototype.read = function (a, b) {
            ...
            this.readers.forEach(function (c) {
                ...
                d ? e.push(f) : 0 < c.configName.indexOf(".") ? g.Point.prototype.setNestedProperty(e,
                    f, c.configName) : e[c.configName] = f
            });
            void 0 === this.name && 2 <= this.readers.length && (f = this.getReferencedColumnIndexes(), 2 <= f.length && (f.shift(), f.sort(function (a, b) {
                return a - b
