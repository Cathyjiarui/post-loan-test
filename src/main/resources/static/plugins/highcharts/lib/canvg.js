...
(function (x, y) {
    ...
    function I(h) {
        ...
        a.Transform =
            function (c) {
                ...
                this.Type.matrix = function (b) {
                    ...
                    this.unapply = function (a) {
                        var b = this.m[0], e = this.m[2], d = this.m[4], c = this.m[1], f = this.m[3], k = this.m[5],
                            h = 1 / (b * (1 * f - 0 * k) - e * (1 * c - 0 * k) + d * (0 * c - 0 * f));
                ...
                for (var e = 0; e <
                c.length; e++) {
                    var d = a.trim(c[e].split("(")[0]), k = c[e].split("(")[1].replace(")", ""),
                        k = new this.Type[d](k);
                    k.type = d;
        ...
        a.Element.svg = function (c) {
            ...
            this.setContext = function (b) {
                ...
                if ("undefined" == typeof this.root) {
                    var e = this.attribute("width").toPixels("x"), d = this.attribute("height").toPixels("y"), c = 0,
                        f = 0;
        ...
        a.Element.rect = function (c) {
            ...
            this.base(c);
            this.path = function (b) {
                var e = this.attribute("x").toPixels("x"), d = this.attribute("y").toPixels("y"),
                    c = this.attribute("width").toPixels("x"), f = this.attribute("height").toPixels("y"),
                    g = this.attribute("rx").toPixels("x"), m = this.attribute("ry").toPixels("y");
        ...
        a.Element.circle = function (c) {
            ...
            this.base(c);
            this.path = function (b) {
                var e = this.attribute("cx").toPixels("x"), d = this.attribute("cy").toPixels("y"),
        ...
        a.Element.ellipse = function (c) {
            ...
            this.path = function (b) {
                var e = (Math.sqrt(2) - 1) / 3 * 4, d = this.attribute("rx").toPixels("x"),
                    c = this.attribute("ry").toPixels("y"), f = this.attribute("cx").toPixels("x"),
                        ...
                        case "a":
                            for (; !d.isCommandOrEnd();) {
                                var m = d.current, r = d.getScalar(), h = d.getScalar(), g = d.getScalar() *
                                    (Math.PI / 180), p = d.getScalar(), n = d.getScalar(), f = d.getAsCurrentPoint(),
                                    l = new a.Point(Math.cos(g) * (m.x - f.x) / 2 + Math.sin(g) * (m.y - f.y) / 2, -Math.sin(g) * (m.x - f.x) / 2 + Math.cos(g) * (m.y - f.y) / 2),
                                    v = Math.pow(l.x, 2) / Math.pow(r, 2) + Math.pow(l.y, 2) / Math.pow(h, 2);
                                ...
                                var q = new a.Point(p * r * l.y /
                                    h, p * -h * l.x / r),
                                    m = new a.Point((m.x + f.x) / 2 + Math.cos(g) * q.x - Math.sin(g) * q.y, (m.y + f.y) / 2 + Math.sin(g) * q.x + Math.cos(g) * q.y),
                                    w = function (a, b) {
                                    ...
                                    }, p = x([1, 0], [(l.x - q.x) / r, (l.y - q.y) / h]),
                                    v = [(l.x - q.x) / r, (l.y - q.y) / h], q = [(-l.x - q.x) / r, (-l.y - q.y) / h],
                                    l = x(v, q);
                                -1 >= w(v, q) && (l = Math.PI);
                                1 <= w(v, q) && (l = 0);
                                v = 1 - n ? 1 : -1;
        ...
        a.Element.pattern = function (c) {
            ...
            this.createPattern = function (b, e) {
                var d = this.attribute("width").toPixels("x", !0), c = this.attribute("height").toPixels("y", !0),
                    f = new a.Element.svg;
        ...
        a.Element.linearGradient = function (c) {
            ...
            this.getGradient = function (a, e) {
                ...
                var k = "objectBoundingBox" == this.gradientUnits() ? c.x() + c.width() * this.attribute("x1").numValue() : this.attribute("x1").toPixels("x"),
                    f = "objectBoundingBox" == this.gradientUnits() ? c.y() + c.height() * this.attribute("y1").numValue() :
        ...
        a.Element.text = function (c) {
            ...
            this.getBoundingBox = function () {
                var b = this.attribute("x").toPixels("x"), c = this.attribute("y").toPixels("y"),
            ...
            this.measureText = function (b) {
                ...
                if (null != c) {
                    ...
                    for (var f = a.ToNumberArray(this.parent.attribute("dx").value), g = 0; g < k.length; g++) {
                        var m = this.getGlyph(c, k, g),
                            d = d + (m.horizAdvX || c.horizAdvX) * b / c.fontFace.unitsPerEm;
        ...
        a.Element.image = function (c) {
            ...
            if ("" != b) {
                ...
                this.renderChildren = function (b) {
                    var c = this.attribute("x").toPixels("x"), d = this.attribute("y").toPixels("y"),
                        m = this.attribute("width").toPixels("x"), n = this.attribute("height").toPixels("y");
                ...
                this.getBoundingBox = function () {
                    var b = this.attribute("x").toPixels("x"), c = this.attribute("y").toPixels("y"),
                        d = this.attribute("width").toPixels("x"), e = this.attribute("height").toPixels("y");
        ...
        a.Element.mask = function (c) {
            ...
            this.apply = function (b, c) {
                var d = this.attribute("x").toPixels("x"), k = this.attribute("y").toPixels("y"),
                    f = this.attribute("width").toPixels("x"), g = this.attribute("height").toPixels("y");
                if (0 == f && 0 == g) {
                    g = new a.BoundingBox;
        ...
        a.Element.filter = function (c) {
            ...
            this.apply = function (a, c) {
                var d = c.getBoundingBox(), k = Math.floor(d.x1), f = Math.floor(d.y1), g = Math.floor(d.width()),
                    d = Math.floor(d.height()), m = c.style("filter").value;
        ...
        a.Element.feColorMatrix = function (c) {
            ...
            this.apply = function (a, c, d, e, h) {
                ...
                for (d = 0; d < h; d++) for (c = 0; c < e; c++) {
                    var u = l.data[d * e * 4 + 4 * c + 0], p = l.data[d * e * 4 + 4 * c + 1],
                        t = l.data[d * e * 4 + 4 * c + 2], v = l.data[d *
        ...
        a.CreateElement = function (c) {
            var b = c.nodeName.replace(/^[^:]+:/, ""), b = b.replace(/\-/g, ""),
    ...
    if ("undefined" != typeof Element.prototype.matches) w =
        ...
        if ("function" ===
            typeof jQuery || "function" === typeof Zepto) w = function (h, a) {
            return $(h).is(a)
        };
